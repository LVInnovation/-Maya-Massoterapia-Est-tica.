...
const findUsablePackage = async (phone: string, service: Service) => {
  ...
  return ((data || []) as ClientPackage[]).find((item) => {
    const total = Number(item.total_sessions || 0);
    const used = Number(item.sessions_used || 0);
    const sameService =
      !item.service_id ||
      String(item.service_id) === String(service.id) ||
      String(item.package_name || '').toLowerCase().includes(service.name.toLowerCase());
    const notExpired = !item.expiration_date || item.expiration_date >= today;
    return sameService && notExpired && total > used;
  }) || null;
};

const debitPackageSession = async (packageItem: ClientPackage, appointmentId: string, serviceName: string) => {
...
const BookingModal = ({
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      const usablePackage = await findUsablePackage(typedPhone, selectedService);
      ...
      if (usablePackage) {
        await debitPackageSession(usablePackage, savedBooking.id, selectedService.name);
      }
