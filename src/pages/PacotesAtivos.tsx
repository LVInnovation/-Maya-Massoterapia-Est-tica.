...
const PacotesAtivos = () => {
  ...
  const loadPackages = useCallback(async () => {
    ...
    try {
      const [packagesResponse, professionalsResponse, sessionsResponse] = await Promise.all([
        ...
        supabase.from('professionals').select('id,name').order('created_at', { ascending: true }),
        supabase.from('package_sessions').select('*').order('used_at', { ascending: false }),
      ])
  ...
  const handleRegisterSession = async (item: ClientPackage) => {
    ...
    try {
      ...
      const { error: historyError } = await supabase.from('package_sessions').insert({
        package_id: item.id,
        used_at: todayKey(),
