...
const PacotesAtivos = () => {
  ...
  const loadPackages = useCallback(async () => {
    ...
    try {
      const [packagesResponse, professionalsResponse, sessionsResponse] = await Promise.all([
        ...
        supabase.from('maya_professional').select('id,name').order('created_at', { ascending: true }),
  ...
  const visiblePackages = useMemo(() => {
    ...
    return packages
      ...
      .filter((item) => {
        ...
        if (!isAdmin) {
          return searchedPhone.length >= 8 && item.normalizedPhone === searchedPhone
        }
  ...
  const handleCreatePackage = async (event: FormEvent) => {
    ...
    try {
      ...
      if (clientId) {
        await supabase.from('maya_clients').update(clientPayload).eq('id', clientId)
      } else {
      ...
      const { error } = await supabase.from('maya_client_packages').insert({
        client_id: clientId,
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="text-sm text-gray-300">
              {isAdmin ? 'Acompanhe clientes com pacotes, sessões usadas e vencimentos' : 'Consulte seus pacotes pelo telefone cadastrado'}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div className="flex-1">
              ...
              <input
                ...
                placeholder={isAdmin ? 'Nome, telefone, pacote ou profissional' : 'Digite seu telefone'}
